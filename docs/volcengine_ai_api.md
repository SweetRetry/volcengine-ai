...
**重要更新**: 现在系统正确关联了数据库任务ID和火山引擎外部任务ID，确保任务状态查询的准确性。

## 任务ID关联机制

系统使用双重任务ID机制：

1. **数据库任务ID**: 系统内部生成的唯一标识符，用于API调用
2. **外部任务ID**: 火山引擎服务返回的任务标识符，用于查询外部服务状态

**工作流程**:
1. 创建任务时，先调用火山引擎API获取外部任务ID
2. 将外部任务ID存储到数据库任务记录中
3. 查询任务状态时，通过数据库任务ID获取外部任务ID，然后查询火山引擎状态
4. 根据外部服务状态更新数据库中的任务状态
